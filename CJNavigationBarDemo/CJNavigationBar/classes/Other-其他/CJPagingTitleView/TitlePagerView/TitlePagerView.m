...
- (void)updatePageIndicatorPosition:(CGFloat)xPosition {
    CGFloat pageIndicatorXPosition = (((xPosition - CJScreenW)/CJScreenW) * (self.width - self.pageIndicator.width))/(self.titleArray.count - 1);
...
-(id)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    for(UIView *subView in subViews){
        if([subView isKindOfClass:[UILabel class]]){ //是要找的图片
